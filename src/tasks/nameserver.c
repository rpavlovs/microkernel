...
void nameserver() {
	debug( DBG_SYS, "> NAMESERVER: start with tid %d", MyTid() );
	...
	FOREVER {
		debug( DBG_SYS, "> NAMESERVER: Block to recieve request" );
		Receive( &sender_tid, (char *) &request, sizeof(request) );

		switch( request.type ) {
		case NAMESERVER_REGISTER_AS_REQUEST:
			debug( DBG_SYS, "> NAMESERVER: RegisterAs request recived" );
		...
		case NAMESERVER_WHO_IS_REQUEST:
			debug( DBG_SYS, "> NAMESERVER: WhoIs request recived" );
			pos = find_entry( request.ns_name, &table );
