...
void schedule_for_wakeup( Wakeup_list *list, int new_tid, int wakeup_time ) {
	...
	bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
		"TIMESERVER: Adding wakeup record for task %d to run at %d", new_tid, wakeup_time );
	bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
		"TIMESERVER: Adding wakeup record for task %d to run at %d", new_tid, wakeup_time );
	...
	bwdebug( DBG_SYS, DELAY_DEBUG_AREA, "TIMESERVER: Adding wakeup record" );
	if( list->size == 0 ) {
		bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
			"TIMESERVER: Adding to the first record to the list" );
		new_record->next_to_wakeup = 0;
		list->first_to_wakeup = new_record;
	}
	else if( wakeup_time < list->first_to_wakeup->wakeup_time ) {
		bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
			"TIMESERVER: Adding to the beginning of the non-empty list" );
		new_record->next_to_wakeup = list->first_to_wakeup;
		list->first_to_wakeup = new_record;
	} else {
		bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
			"TIMESERVER: Adding in the middle of the list" );
		Wakeup_record *wake_after = list->first_to_wakeup; 
		Wakeup_record *wake_before = wake_after->next_to_wakeup;
		
		bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
			"TIMESERVER: Adding between %d and %d", wake_after, wake_before );
		
		while( wake_before != 0 && wakeup_time > wake_after->wakeup_time ){
			
			wake_after = wake_after->next_to_wakeup;
			wake_before = wake_after->next_to_wakeup;

			bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
				"TIMESERVER: correct: Adding between %d and %d", wake_after, wake_before );
		}
		...
		bwdebug( DBG_SYS, DELAY_DEBUG_AREA,
			"TIMESERVER: Adding to the middle of the list done" );
	}
	
	list->size++;

	bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
		"TIMESERVER: task %d is scheduled to run at %d ticks",
...
int get_tid_to_wakeup( Wakeup_list *list, int current_time ) {	
	...
	bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
		"TIMESERVER: list->size = %d, list->first_to_wakeup = %d ",
		list->size, list->first_to_wakeup );
...
void clock_tick_notifier() {
	...
	FOREVER {
		...
		bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
			"TICK_NOTIFIER: got a tick. Sending a poke to task %d", timeserver_tid);
...
void timeserver() {
	...
	FOREVER {
		bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
			"TIMESERVER: listening as task %d...", mytid );

		Receive( &sender_tid, (char *) &request, sizeof(request) );
		switch( request.type ) {
		case TICK_NOTIFICATION:
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: tick notification recieved from task %d. [time: %d]",
			...
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: tick notification reply dispatched" );
			Reply( sender_tid, (char *) 0, 0 );
			break;
		case TIME_REQUEST:
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: time request recieved from task %d", sender_tid );
		...
		case DELAY_REQUEST:
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: delay request for %d ticks recieved from task %d",
				request.num, sender_tid );
			schedule_for_wakeup( &list, sender_tid, current_time + request.num );
			break;
		case DELAY_UNTIL_REQUEST:
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: delay request until %d ticks recieved from task %d",
				request.num, sender_tid );
			schedule_for_wakeup( &list, sender_tid, request.num );
			break;
		default:
			bwdebug( DBG_SYS, TIMESERVER_DEBUG_AREA,
				"TIMESERVER: *FATAL* unexpected message [type: %d from: %d]",
