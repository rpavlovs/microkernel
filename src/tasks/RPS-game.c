...
void server_entry_point()
{
    ...
    bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "STARTING RPS Server. " ); 
    FOREVER{
        ...
        bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "entering switch." );
        switch ( operation_id ) {
        case RPS_OP_SIGNUP:
            bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "Signup. " );
            ...
            break;
        case RPS_OP_PLAY:
            bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "Playss. " );
            ...
            break;
        case RPS_OP_QUIT:
            bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "Quit. " );
            // Check if the sender is one of the clients currently playing. 
            if ( !( sender_tid == server_data.player_one.tid || sender_tid == server_data.player_two.tid ) )
            {
                    reply_client( sender_tid, RPS_ERROR ); 
            ...
            {
                bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "Exiting game server");
                Exit(); 
            }
            
            break;
        default:
            bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "RPS ERROR: Invalid Operation ID. " ); 
            break;
        }
    }
    
    bwdebug( DBG_USR, RPS_GAME_DEBUG_AREA, "EXITING RPS Server. " ); 
}
