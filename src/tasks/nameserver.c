...
void nameserver() {
	bwdebug( DBG_SYS, "NAMESERVER: enters" );
	...
	FOREVER {
		bwdebug( DBG_SYS, "NAMESERVER: listening for a request" );
		Receive( &sender_tid, (char *) &request, sizeof(request) );

		switch( request.type ) {
		case NAMESERVER_REGISTER_AS_REQUEST:
			bwdebug( DBG_SYS, "NAMESERVER: RegisterAs request recived from %d", sender_tid );
			pos = find_entry( request.ns_name, &table );
			if ( pos == -1 ) {
				bwdebug( DBG_SYS, "NAMESERVER: new ns record required" );
				pos = table.size++;
		...
		case NAMESERVER_WHO_IS_REQUEST:
			bwdebug( DBG_SYS, "NAMESERVER: WhoIs request recived from %d", sender_tid );
			pos = find_entry( request.ns_name, &table );
			reply.num = (pos < 0 ? NS_ERROR_TASK_NOT_FOUND : table.entrie[pos].tid);
			break;
		default:
			bwdebug( DBG_SYS, "NAMESERVER: Nameserver received unexpected "
				"message [type: %d from: %d]", request.type, sender_tid );
			reply.num = ERROR_WRONG_MESSAGE_TYPE;
		}

		if( reply.num < 0 )
			bwdebug( DBG_SYS, "NAMESERVER: Could not find [%s] requested "
				"by task %d", request.ns_name, sender_tid );
		
		bwdebug( DBG_SYS, "NAMESERVER: replying to: %d [msg: %d]",
